...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center space-x-2 text-sm text-sogan/60">
            ...
            <i class="fas fa-chevron-right text-xs"></i>
            <span class="text-emas">Edit: {{ $category->name }}</span>
        </div>
    </div>

    <!-- Header -->
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl md:text-3xl font-playfair font-bold text-sogan">Edit Kategori</h1>
        ...
        <form action="{{ route('categories.update', $category) }}" method="POST">
            ...
            <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                <label class="flex items-center space-x-3 cursor-pointer">
                    ...
                    <span class="text-sm text-sogan">Kategori Aktif</span>
                </label>
                <p class="text-xs text-sogan/50 mt-2 ml-8">
                    Nonaktifkan jika kategori tidak digunakan
                </p>
            </div>

            <!-- Informasi Sistem -->
            <div class="mb-6 p-4 bg-emas/5 rounded-lg">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        <span class="text-sogan/60">Dibuat:</span>
                        <span class="ml-2 text-sogan font-medium">{{ $category->created_at->format('d M Y H:i') }}</span>
                    </div>
                    <div>
                        <span class="text-sogan/60">Diupdate:</span>
                        <span class="ml-2 text-sogan font-medium">{{ $category->updated_at->format('d M Y H:i') }}</span>
                    </div>
                    <div class="col-span-2">
                        <span class="text-sogan/60">Jumlah Sparepart:</span>
                        <span class="ml-2 text-emas font-semibold">{{ $category->spareparts_count ?? $category->spareparts()->count() }} item</span>
                    </div>
                </div>
            </div>
