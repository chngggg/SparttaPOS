...
<div class="mb-8 relative">
    ...
    <p class="text-sogan/70">Selamat datang kembali, <span class="font-semibold">{{ auth()->user()->name }}</span>! Berikut ringkasan inventory hari ini.</p>
</div>
    ...
    <div class="stat-card-gradient bg-white rounded-xl shadow-md p-6 hover-lift">
        <div class="flex items-start justify-between">
            <div>
                ...
                <h3 class="text-3xl font-bold text-sogan">{{ number_format($totalSparepart) }}</h3>
        ...
        <div class="mt-4 pt-4 border-t border-emas/20">
            <div class="flex items-center text-sm">
                <span class="text-sogan/70">{{ $totalKategori ?? '8' }} kategori tersedia</span>
    ...
    <div class="stat-card-gradient bg-white rounded-xl shadow-md p-6 hover-lift">
        <div class="flex items-start justify-between">
            <div>
                ...
                <h3 class="text-3xl font-bold {{ $lowStockItems > 0 ? 'text-red-600' : 'text-green-600' }}">
                    {{ $lowStockItems }}
                </h3>
                @if($lowStockItems > 0)
            ...
            <div class="w-12 h-12 bg-gradient-to-br {{ $lowStockItems > 0 ? 'from-red-500 to-red-600' : 'from-green-500 to-green-600' }} rounded-lg flex items-center justify-center shadow-lg">
                <i class="fas fa-{{ $lowStockItems > 0 ? 'exclamation' : 'check' }} text-white text-xl"></i>
            </div>
        </div>
        <div class="mt-4 pt-4 border-t border-emas/20">
            <div class="flex items-center text-sm">
                <span class="text-sogan/70">{{ $lowStockSpareparts->count() }} item di bawah minimum</span>
                <i class="fas fa-chevron-right ml-auto text-emas"></i>
            </div>
        </div>
    </div>
    ...
    <div class="stat-card-gradient bg-white rounded-xl shadow-md p-6 hover-lift">
        <div class="flex items-start justify-between">
            <div>
                ...
                <h3 class="text-3xl font-bold text-emas">Rp {{ number_format($todaySales, 0, ',', '.') }}</h3>
                <p class="text-xs text-green-600 mt-2">
        ...
        <div class="mt-4 pt-4 border-t border-emas/20">
            <div class="flex items-center text-sm">
                ...
                <span class="ml-auto text-emas">{{ round(($todaySales/3000000)*100) }}%</span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-1.5 mt-2">
                <div class="bg-emas h-1.5 rounded-full" style="width: {{ min(($todaySales/3000000)*100, 100) }}%"></div>
            </div>
        </div>
    </div>
    ...
    <div class="stat-card-gradient bg-white rounded-xl shadow-md p-6 hover-lift">
        <div class="flex items-start justify-between">
            <div>
                ...
                <h3 class="text-3xl font-bold text-sogan">{{ $todayTransactions }}</h3>
        ...
        <div class="mt-4 pt-4 border-t border-emas/20">
            <div class="flex items-center text-sm">
                <span class="text-sogan/70">Rp {{ number_format($totalBulanIni ?? 12500000, 0, ',', '.') }} total bulan ini</span>
                <i class="fas fa-chevron-right ml-auto text-emas"></i>
            </div>
        </div>
    </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        <div class="flex items-center justify-between mb-6">
            ...
            <span class="bg-red-100 text-red-600 text-xs px-2 py-1 rounded-full">{{ $lowStockSpareparts->count() }} item</span>
            @endif
        </div>

        <div class="space-y-4">
            @forelse($lowStockSpareparts as $item)
            <div class="flex items-center justify-between p-3 bg-gradient-to-r from-gray-50 to-white rounded-lg hover:shadow-md transition-all duration-300 group">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <p class="font-medium text-sm text-sogan">{{ $item->name }}</p>
                        <p class="text-xs text-sogan/60">Kode: {{ $item->code }}</p>
                    </div>
                </div>
                <div class="text-right">
                    <span class="text-sm font-semibold text-red-600">{{ $item->stock }} pcs</span>
                    <p class="text-xs text-sogan/50">Min: {{ $item->min_stock }}</p>
                </div>
            </div>
            @empty
...
<div class="bg-white rounded-xl shadow-md p-6">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-xs text-sogan/50 mt-1">{{ $todayTransactions }} transaksi hari ini</p>
        </div>
        ...
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-100 hover:bg-emas/5 transition group">
                    <td class="py-3 px-4 font-medium">{{ $transaction['invoice'] }}</td>
                    <td class="py-3 px-4">
                    ...
                    <td class="py-3 px-4">{{ $transaction['date']->format('d/m/Y H:i') }}</td>
                    <td class="py-3 px-4 font-semibold">Rp {{ number_format($transaction['total'], 0, ',', '.') }}</td>
                    <td class="py-3 px-4">
    ...
    <div class="mt-4 pt-4 border-t border-emas/20 flex justify-between items-center">
        <p class="text-xs text-sogan/50">
            Menampilkan {{ $recentTransactions->count() }} transaksi terbaru
        </p>
