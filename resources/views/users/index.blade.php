...
<div class="mb-8">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
        ...
        <a href="{{ route('users.create') }}" class="mt-4 md:mt-0 inline-flex items-center px-4 py-2 bg-emas text-sogan rounded-lg hover:bg-emas-dark transition">
    ...
    <div class="bg-white rounded-xl shadow-md p-4 border-l-4 border-purple-600">
        <div class="flex items-center">
            ...
            <div class="ml-3">
                ...
                <p class="text-xl font-bold">{{ $users->where('role', 'super_admin')->count() }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-4 border-l-4 border-emas">
        <div class="flex items-center">
            ...
            <div class="ml-3">
                ...
                <p class="text-xl font-bold">{{ $users->where('role', 'admin')->count() }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-4 border-l-4 border-sogan">
        <div class="flex items-center">
            ...
            <div class="ml-3">
                ...
                <p class="text-xl font-bold">{{ $users->where('role', 'kasir')->count() }}</p>
            </div>
        </div>
    </div>
        ...
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-100 hover:bg-emas/5 transition">
                    <td class="py-4 px-6">
                        <div class="flex items-center">
                            <div class="w-10 h-10 {{ $user->role_color }} rounded-full flex items-center justify-center text-white">
                                {{ strtoupper(substr($user->name, 0, 1)) }}
                            </div>
                            <div class="ml-3">
                                <p class="font-medium">{{ $user->name }}</p>
                                <p class="text-sm text-sogan/60">{{ $user->email }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="py-4 px-6">
                        <span class="px-3 py-1 {{ $user->role_color }} text-white rounded-full text-xs">
                            {{ $user->role_name }}
                        </span>
                    </td>
                    <td class="py-4 px-6">
                    ...
                    <td class="py-4 px-6">
                        <button onclick="toggleActive({{ $user->id }})"
                            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none {{ $user->is_active ? 'bg-emas' : 'bg-gray-300' }}">
                        ...
                        <span class="ml-2 text-sm {{ $user->is_active ? 'text-green-600' : 'text-red-600' }}">
                            {{ $user->is_active ? 'Aktif' : 'Nonaktif' }}
                        </span>
                    </td>
                    <td class="py-4 px-6">
                        <div class="flex space-x-2">
                            <a href="{{ route('users.edit', $user) }}" class="text-sogan/50 hover:text-emas transition">
                                <i class="fas fa-edit"></i>
                            </a>
                            @if(Auth::user()->isSuperAdmin() && !$user->isSuperAdmin())
                            <button onclick="deleteUser({{ $user->id }})" class="text-sogan/50 hover:text-red-600 transition">
